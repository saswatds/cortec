...
const typedoc = JSON.parse(fs.readFileSync('./typedoc.json').toString());
typedoc.entryPoints = [];
...
(async function() {
    ...
    for (const name in workspaces) {
        ...
        const tsDocPath = path.resolve(location, 'typedoc.json');

        if (fs.existsSync(tsconfigPath)) {
            config.references.push({
                path: workspace.location,
            });
            typedoc.entryPoints.push(workspace.location);
    ...
    fs.writeFileSync('typedoc.json', JSON.stringify(typedoc, undefined, 2));
})();